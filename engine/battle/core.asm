...
;	ld a, [wObtainedBadges]     badgeboostfix attempt
;	ld b, a						badgeboostfix attempt
;	ld hl, wBattleMonAttack		badgeboostfix attempt
;	ld c, $4
...
;	jr nz, .loop
	...
	ld b, a ;					badgeboostfix attempt
	ld hl, wBattleMonAttack ;	badgeboostfix attempt
	bit BIT_BOULDERBADGE, b ;	badgeboostfix attempt
	call nz, ApplyBoostToStat ;	badgeboostfix attempt
	ld hl, wBattleMonSpeed ;	badgeboostfix attempt
	bit BIT_THUNDERBADGE, b ;	badgeboostfix attempt
	call nz, ApplyBoostToStat ;	badgeboostfix attempt
	ld hl, wBattleMonDefense ;	badgeboostfix attempt
	bit BIT_SOULBADGE, b ;		badgeboostfix attempt
	call nz, ApplyBoostToStat ;	badgeboostfix attempt
	ld hl, wBattleMonSpecial ;	badgeboostfix attempt
	bit BIT_VOLCANOBADGE, b ;	badgeboostfix attempt
	call nz, ApplyBoostToStat ; badgeboostfix attempt
	ret

; multiply stat at hl by 1.125
; cap stat at MAX_STAT_VALUE
ApplyBoostToStat: ; .applyBoostToStat badgeboostfix attempt
	ld a, [hli]
	ld d, a
	ld e, [hl]
	srl d
